...
def votes_for_money(money: int):
    # quadratic voting
    return round(math.sqrt(money))


def vote(proposal_type: int, proposal_preferences: [int], voting_money):
    money = 0
    vote_yes = 0
    vote_no = 0
    conviction = proposal_preferences.count(proposal_type)
    money_part = conviction * voting_money/5
    votes = votes_for_money(money_part)
    # hack: if no interest vote no
    if votes == 0:
        money_part = voting_money/5
        vote_no = votes_for_money(money_part)
    else:
        vote_yes = votes
    return (money_part, vote_yes, vote_no)