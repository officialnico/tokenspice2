...
@dataclass
class EWAgentWallet():
    # __slots__ = ['USD', 'OCEAN', 'private_key', '_web3wallet', '_total_USD_in', '_total_OCEAN_in', '_cached_OCEAN_base' ]
    _web3wallet: field(init=False)
    OCEAN: float 
    USD: float = 0.0
    private_key: str = None
    _total_USD_in: float = field(default=0.0)
    _total_OCEAN_in: float = field(default=0.0)
    _cached_OCEAN_base: typing.Union[int,None] = None
    ...
    def __post_init__(self):
        if self.private_key is None:
            ...
            self._web3wallet = web3wallet.Web3Wallet(self.private_key)
        ...
        value_base = toBase18(self.OCEAN)
        #OCEAN
        globaltokens.mintOCEAN(self._web3wallet.address,value_base)

        #amount 
        self._total_USD_in:float = self.USD
        self._total_OCEAN_in:float = self.OCEAN
    ...
    def withdrawUSD(self, amt: float) -> None:
        assert amt >= 0.0
        if amt > 0.0 and self.USD > 0.0:
            tol = 1e-12
            if (1.0 - tol) <= amt/self.USD <= (1.0 + tol):
                self.USD = amt #avoid floating point roundoff
        if amt > self.USD:
            amt = round(amt, 12)
        if amt > self.USD:
    ...
    def transferUSD(self, dst_wallet, amt: float) -> None:
        assert isinstance(dst_wallet, EWAgentWallet)
    ...
    def transferOCEAN(self, dst_wallet, amt: float) -> None:
        assert isinstance(dst_wallet, EWAgentWallet) or \
